...
def temperature_distribution(env, output_dir):
    fig10, ax10 = plt.subplots(ncols=3, sharey=True, sharex=True,figsize=(15, 5))
    c18 = env[(env['temp'] == 18)]
    # c18 = c18[(c18['DAP'] != 0)]
    sns.regplot(x='DAP', y='leaf_cnt', data=c18, line_kws={'color': '#252525'}, scatter_kws={'s':50, 'color':'#034e7b'}, marker='s', ax=ax10[0])
    ax10[0].set_ylabel('Leaf Number')
    ax10[0].set_xlabel('DAP ($day$)')
    ax10[0].set(xlim=(0, 31))
    ax10[0].set(ylim=(0, 13))
    ax10[0].set_title('18°C Leaf Number')

    c21 = env[env['temp'] == 21]
    sns.regplot(x='DAP', y='leaf_cnt', data=c21, line_kws={'color': '#252525'}, scatter_kws={'s':50, 'color':'#034e7b'}, marker='s', ax=ax10[1])
    ax10[1].set(xlim=(0, 31))
    ax10[1].set(ylim=(0, 13))
    ax10[1].set_xlabel('DAP ($day$)')
    ax10[1].set(ylabel=None)
    ax10[1].set_title('21°C Leaf Number')

    c24 = env[env['temp'] == 24]
    sns.regplot(x='DAP', y='leaf_cnt', data=c24, line_kws={'color': '#252525'}, scatter_kws={'s': 50, 'color': '#034e7b'}, marker='s', ax=ax10[2])
    ax10[2].set(xlim=(0, 31))
    ax10[2].set(ylim=(0, 13))
    ax10[2].set_xlabel('DAP ($day$)')
    ax10[2].set(ylabel=None)
    ax10[2].set_title('24°C Leaf Number')
    fig10.tight_layout()
    fig10.savefig(fname=f'{output_dir}/temperature_distribution.png', bbox_inches='tight', pad_inches=0.2)

def c18_length(env, output_dir):
    fig11, ax11 = plt.subplots()
    l18 = env[env['temp'] == 18]
    sns.regplot(x='leaf_cnt', y='leaf_length', data=l18, line_kws={'color': '#252525'}, scatter_kws={'s':50, 'color':'#034e7b'}, marker='s')
    ax11.set_title('18°C Leaf Length')
    ax11.set_ylabel('Leaf Length ($cm$)')
    ax11.set_xlabel('Leaf Number')
    fig11.savefig(f"{output_dir}/18°C Leaf Length.png")

def c21_length(env, output_dir):
    fig11, ax11 = plt.subplots()
    l21 = env[env['temp'] == 21]
    sns.regplot(x='leaf_cnt', y='leaf_length', data=l21, line_kws={'color': '#252525'}, scatter_kws={'s':50, 'color':'#034e7b'}, marker='s')
    ax11.set_title('21°C Leaf Length')
    ax11.set_ylabel('Leaf Length ($cm$)')
    ax11.set_xlabel('Leaf Number')
    fig11.savefig(f"{output_dir}/21°C Leaf Length.png")


def c24_length(env, output_dir):
    fig11, ax11 = plt.subplots()
    l24 = env[env['temp'] == 24]
    sns.regplot(x='leaf_cnt', y='leaf_length', data=l24, line_kws={'color': '#252525'}, scatter_kws={'s':50, 'color':'#034e7b'}, marker='s')
    ax11.set_title('24°C Leaf Length')
    ax11.set_ylabel('Leaf Length ($cm$)')
    ax11.set_xlabel('Leaf Number')
    fig11.savefig(f"{output_dir}/24°C Leaf Length.png")


def length_distribution(env, output_dir):
    fig12, ax12 = plt.subplots(ncols=3, sharey=True, sharex=True,figsize=(15, 5))
    l18 = env[env['temp'] == 18]
    sns.regplot(x='leaf_cnt', y='leaf_length', data=l18, line_kws={'color': '#252525'}, scatter_kws={'s':50, 'color':'#034e7b'}, marker='s', ax=ax12[0])
    ax12[0].set_ylabel('Leaf Length ($cm$)')
    ax12[0].set_xlabel('Leaf Number')
    ax12[0].set(xlim=(0, 12))
    ax12[0].set(ylim=(0, 14))
    ax12[0].set_title('18°C Leaf Length')

    l21 = env[env['temp'] == 21]
    sns.regplot(x='leaf_cnt', y='leaf_length', data=l21, line_kws={'color': '#252525'}, scatter_kws={'s':50, 'color':'#034e7b'}, marker='s', ax=ax12[1])
    ax12[1].set(xlim=(0, 12))
    ax12[1].set(ylim=(0, 14))
    ax12[1].set_xlabel('Leaf Number')
    ax12[1].set(ylabel=None)
    ax12[1].set_title('21°C Leaf Length')

    l24 = env[env['temp'] == 24]
    sns.regplot(x='leaf_cnt', y='leaf_length', data=l24, line_kws={'color': '#252525'}, scatter_kws={'s':50, 'color':'#034e7b'}, marker='s', ax=ax12[2])
    ax12[2].set(xlim=(0, 12))
    ax12[2].set(ylim=(0, 14))
    ax12[2].set_xlabel('Leaf Number')
    ax12[2].set(ylabel=None)
    ax12[2].set_title('24°C Leaf Length')
    fig12.tight_layout()
    fig12.savefig(fname=f'{output_dir}/length_distribution.png', bbox_inches='tight', pad_inches=0.2)



def main():
    env = pd.read_csv('kale_env_v2.csv', parse_dates=['date'])
    ln18 = pd.read_csv('18_leaf_number_v2.csv')
    ln21 = pd.read_csv('21_leaf_number_v2.csv')
    ln24 = pd.read_csv('24_leaf_number_v2.csv')

    output_dir = "../output/leaf_number_increase_v2"
    ...
    temperature_distribution(env, output_dir)
    c18_length(env, output_dir)
    c21_length(env, output_dir)
    c24_length(env, output_dir)
    length_distribution(env, output_dir)
